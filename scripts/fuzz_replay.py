...
def replay(config_path: str, queue_dir: str, crash_dir: str, hang_dir: str):
    ...
    path_stats = os.path.join(os.path.dirname(queue_dir), "fuzzer_stats")
    with open(path_stats, 'r') as ff:
        for line in ff.readlines():
            if 'execs_per_sec' in line:
                exec_per_sec = line.split(':')[1].strip()
                break
    print(f"cmd: {cmd}")
    os.system(cmd)
...
def main():
    ...
    if len(fuzz_dirs) == 1:
        replay(
            os.path.join(FUZZ_BASE_PATH, sys.argv[1], fuzz_dirs[0], "default", "config.yml"),
            os.path.join(FUZZ_BASE_PATH, sys.argv[1], fuzz_dirs[0], "default", "queue"),
            os.path.join(FUZZ_BASE_PATH, sys.argv[1], fuzz_dirs[0], "default", "crashes"),
            os.path.join(FUZZ_BASE_PATH, sys.argv[1], fuzz_dirs[0], "default", "hangs")
        )
        return
    if len(fuzz_dirs) == 0:
    ...
    if id.isalnum() and int(id) >= 1 and int(id) <= len(fuzz_dirs):
        replay(
            os.path.join(FUZZ_BASE_PATH, sys.argv[1], fuzz_dirs[int(id)-1], "default", "config.yml"),
            os.path.join(FUZZ_BASE_PATH, sys.argv[1], fuzz_dirs[int(id)-1], "default", "queue"),
            os.path.join(FUZZ_BASE_PATH, sys.argv[1], fuzz_dirs[int(id)-1], "default", "crashes"),
            os.path.join(FUZZ_BASE_PATH, sys.argv[1], fuzz_dirs[int(id)-1], "default", "hangs")
        )
        return
    else:
        print("invalid id")
