...
void afl_fsrv_start(afl_forkserver_t *fsrv, char **argv,
  ...
  fsrv->fsrv_pid = -1;
  fsrv->real_map_size = fsrv->map_size = get_coverage_size();
  fsrv->use_shmem_fuzz = 1;
  int ok = init_execution(fsrv->target_path, ((afl_state_t *)(fsrv->afl_ptr))->out_dir);
  if (!ok) {
    FATAL("failed to init kvm engine");
    return;
  }
  return;
#endif
...
#ifdef KVM_HACK
void save_buffer_to_file(char *path, const void *buffer, size_t len) {
  char file_path[1024];

  sprintf(file_path, "%s/crash.bin", path);
  FILE *fp = fopen(file_path, "wb");
  if (!fp) {
      perror("fopen");
      FATAL("kvm internal crash: falied to save crash input");
  }
...
#ifdef KVM_HACK
  reset_execution(fsrv->shmem_fuzz, *fsrv->shmem_fuzz_len);
  int ret_val = start_execution(timeout * 1000);
  fsrv->total_execs++;
  switch (ret_val) {
    case 0:
      return FSRV_RUN_OK;
    ...
    default:
      save_buffer_to_file(
        ((afl_state_t *)(fsrv->afl_ptr))->out_dir,
        fsrv->shmem_fuzz, *fsrv->shmem_fuzz_len
      );
      FATAL(
        "kvm internal crash: crash input saved to crash.bin, "
        "error PC = 0x%08x", error_pc()
      );
  }
  return FSRV_RUN_OK;
#endif
